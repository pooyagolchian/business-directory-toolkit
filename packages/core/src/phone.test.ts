...
describe("normalizePhone", () => {
  test("converts a Dubai landline to E.164", () => {
    expect(normalizePhone("04 577 6680")?.e164).toBe("+97145776680");
  });

  test("converts an Etisalat mobile to E.164", () => {
    expect(normalizePhone("052 253 3290")?.e164).toBe("+971522533290");
  });

  test("converts a du mobile to E.164", () => {
    expect(normalizePhone("050 786 8343")?.e164).toBe("+971507868343");
  });

  test("tags a 04 number as a landline", () => {
    expect(normalizePhone("04 577 6680")?.type).toBe("landline");
  });

  test("tags a 05x number as a mobile", () => {
    expect(normalizePhone("052 253 3290")?.type).toBe("mobile");
  });

  test("accepts a number that is already in international format", () => {
    expect(normalizePhone("+971 4 577 6680")?.e164).toBe("+97145776680");
  });

  test("preserves the original string so the source stays auditable", () => {
    expect(normalizePhone("04 577 6680")?.raw).toBe("04 577 6680");
  });
